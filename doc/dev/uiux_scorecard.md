...
| 전투 씬 UX | 7.5 | 9.5 | CG가 UI 뒤에 놓이고 불필요한 전투 로그 패널은 숨김. 릴 회전 중 시간 기반 ticker가 표시되고, STOP/선택 적용/도주 동선이 하단 릴 패널과 분리되어 겹침이 줄어듦. 승리 후에는 새 테스트 전투를 시작하지 않고 복귀 선택 패널을 보여줌. | 전투 결과 피드백 애니메이션/사운드가 없어서 상용 수준의 타격감은 아직 부족. |
...
- 적용:
  - 전투 릴 카드가 회전 중일 때 현재 시간 기반으로 숫자/스킬 preview 중심 인덱스를 바꾸도록 수정해 정지 화면처럼 보이지 않게 함.
  - 레거시 전투 시스템 이미지와 웹게임 `renderCombat.js`의 3칸 ticker 구조를 참고해 릴 카드 내부 window를 더 압축된 3단 구조로 정리함.
  - 전투 명령 패널을 중앙 오버레이에서 하단 릴 트레이 위쪽으로 이동하고, 릴 패널 폭을 넓혀 4-5개 릴이 잘리지 않도록 함.
