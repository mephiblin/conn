...
| 스킬 주사위 장착 UX | 7.5 | 9.5 | 스킬 카드 드래그/드롭과 클릭 장착은 동작함. 보유/장착/여유 수량과 선택된 스킬 상태가 표시되고, 한손/방패/양손 장비별 스킬군 보정이 전투와 UI에 연결됨. 주사위 face 슬롯이 선택 스킬 기준으로 `Drop`/`Replace`/`Same`/`Clear` 상태와 색상을 구분하고 `Drop here`/`Would replace`/`Click clears`/`Already placed` 행동 힌트까지 표시해 장착 결과를 더 즉시 읽을 수 있음. | 실제 Game view에서 드래그 중 커서 위치/슬롯 hover 하이라이트가 안정적으로 보이는지 확인해야 10점. |
| 캐릭터 생성 | 7.0 | 9.5 | 개발 문구를 제거하고 초상화별 플레이어용 프로필 설명으로 교체함. 초상화별 시작 무기/스킬 프리셋과 추천 빌드 설명이 실제 새 게임 장비/스킬 장착 상태에 연결됨. Vitality가 실제 시작 Max HP에 반영되고, 선택된 프리셋은 `Selected preset` 라벨과 초상화 프레임 색상으로 구분됨. 캐릭터 생성 스탯 패널에도 같은 규칙의 Starting HP가 표시됨. | Game view 기준 선택 피드백, 직업별 장기 성장 곡선, 최종 밸런스 플레이테스트가 필요. |
...
| 전투 씬 UX | 7.5 | 9.5 | CG가 UI 뒤에 놓이고 불필요한 전투 로그 패널은 숨김. 릴 회전 중 시간 기반 ticker가 표시되고, STOP/선택 적용/도주 동선이 하단 릴 패널과 분리되어 겹침이 줄어듦. 턴 결과는 피해/방어/회복/반격 요약과 `SPIN`/`READY`/`IMPACT`/`DANGER`/`VICTORY` 배너 색상으로 명령 패널 상단에 분리 표시됨. 배너에는 결과 종류별 pulse scale과 audio cue 매핑이 추가되어 피해/위험/승리 상태를 연출 시스템에서 구분 가능함. 승리 후에는 새 테스트 전투를 시작하지 않고 복귀 선택 패널을 보여줌. | 실제 Game view에서 pulse 강도와 화면 겹침을 확인하고, 오디오 에셋을 cue에 매핑해야 10점. |
...
### 2026-06-03 캐릭터 프리셋 선택 피드백 루프

- 적용:
  - 캐릭터 생성 초상화 프레임 색상을 프리셋별로 분리해 현재 선택된 빌드를 더 즉시 읽을 수 있게 함.
  - 초상화 하단에 `Selected preset | {이름} | Starting HP {값}` 라벨을 추가해 선택 상태와 시작 HP를 한 줄로 확인하게 함.
  - `CharacterPresetSelectionLabel`과 `CharacterPresetFrameColor` helper를 추가하고, 프리셋별 라벨/색상 회귀 검증을 보강함.
- 점수 변경:
  - 캐릭터 생성: 9.5 유지. 선택 피드백은 보강됐지만 10점에는 실제 Game view 확인, 직업별 장기 성장 곡선, 최종 밸런스 플레이테스트가 필요함.
- 검증:
  - `git diff --check` 통과.
  - Unity `RuntimeCoreRulesTests`: 59/59 통과 (`tmp/runtimecore-character-preset-selection-results.xml`, failed=0).
  - 커밋: `340b993`
...
### 2026-06-03 스킬 face 슬롯 행동 힌트 루프

- 적용:
  - 주사위 face 슬롯의 상태 라벨에 더해 `Click clears`/`Choose a skill`/`Drop here`/`Would replace`/`Already placed` 행동 힌트를 추가함.
  - 기본공격/장착 스킬 슬롯 모두 같은 힌트 라인을 표시해 클릭/드롭 전 결과를 읽을 수 있게 함.
  - `SkillDropSlotActionHint` helper와 회귀 테스트를 추가함.
- 점수 변경:
  - 스킬 주사위 장착 UX: 9.5 유지. 실제 Game view에서 드래그 중 커서 위치와 슬롯 hover 하이라이트가 안정적인지 확인해야 10점.
- 검증:
  - `git diff --check` 통과.
  - Unity `RuntimeCoreRulesTests`: 59/59 통과 (`tmp/runtimecore-skill-slot-hints-results.xml`, failed=0).
  - 커밋: `d10b92f`

### 2026-06-03 전투 피드백 pulse/cue 루프

- 적용:
  - `CombatFeedbackPulseScale`을 추가해 `enemy`/`victory`/`exchange` 배너가 `ready`보다 강한 pulse 값을 갖도록 함.
  - `CombatFeedbackAudioCue`를 추가해 `combat_impact`/`combat_danger`/`combat_victory` 같은 결과별 오디오 cue 이름을 고정함.
  - `CombatFeedbackPulseAnimator`를 배너에 붙여 런타임에서 결과 배너의 scale/alpha가 시간 기반으로 움직이게 함.
  - `CombatFeedbackKindsExposeDistinctImpactBadgesAndColors` 테스트에 pulse 강도와 audio cue 회귀 검증을 추가함.
- 점수 변경:
  - 전투 씬 UX: 9.5 유지. 코드상 연출 hook은 생겼지만 실제 Game view pulse 강도와 오디오 에셋 매핑 검증이 필요함.
- 검증:
  - `git diff --check` 통과.
  - Unity `RuntimeCoreRulesTests`: 59/59 통과 (`tmp/runtimecore-combat-feedback-pulse-results.xml`, failed=0).
  - 커밋: `96ded16`
